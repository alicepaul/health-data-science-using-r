...
We can also extract the estimated regression coefficients from the model using the `coef()` function or by using the `tbl_regression()` function from the `gtsummary` package. This function puts the coefficient estimates, the 95% confidence intervals for those estimates, and the p-values for those estimates in a data frame. You can also change how many decimals are displayed in the resulting table using `fmt_number()`. 

```{r}
tbl_regression(adj_model) %>% as_gt() %>% fmt_number(decimals = 3) %>% 
  gt:::as.tags.gt_tbl()
```
...
```{r}
pred_y <- exp(predict(model_interaction))
```
...
We can again use the `tbl_regression()` function from the `gtsummary` package to display the estimated coefficients. This time we add the `exponentiate = TRUE` argument in order to exponentiate our coefficients so we can interpret them as estimated change in odds rather than log odds. For example, those who answered at school have double the estimated odds of reporting e-cigarette use compared to those who took the survey at home/other, adjusting for grade, sex, and race and ethnicity. 

```{r}
tbl_regression(mod_start, exponentiate = TRUE) %>% as_gt() %>% 
  gt:::as.tags.gt_tbl()
```

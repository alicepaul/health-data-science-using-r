...
In this chapter, we will demonstrate a short exploratory analysis as a case study. This case study will focus on Covid-19 cases and deaths during 2020 using the `covidcases` and `mobility` datasets from the **HDSinRdata** package. A new package that will be used in this case study is **usmap** package [@R-usmap], which allows us to easy create spatial plots of the United States. 
...
First, we look at the columns in our data. We convert the date columns in the mobility data to be recognized as as a date using the `as.Date()` function. The `covidcases` data has the week number of 2020. We will create a similar column for the mobility data.

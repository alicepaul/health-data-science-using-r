...
We may also be interested in the order of the values. The `sort()`\index{R functions!sort()@\texttt{sort()}} function sorts the values of a vector, whereas the `order()`\index{R functions!order()@\texttt{order()}} function returns the permutation of the elements to be in sorted order. The last line of following code sorts the days of the week from smallest to largest rain value.

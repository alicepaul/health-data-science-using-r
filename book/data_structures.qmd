...
To create all of these objects, we used the assignment operator `<-`, which we discussed in [Chapter -@sec-intro-to-r]. You may see code elsewhere that uses an `=` instead. While `=` can also be used for assignment, it is more standard practice to use `<-`.

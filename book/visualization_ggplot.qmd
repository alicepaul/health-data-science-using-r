...
The **RColorBrewer** package [@R-RColorBrewer] contains several default palettes to choose from, shown in the following output. You can also create your own palette using the `brewer.pal()` function from this package. To visualize a palette you can use the available [online tool](https://colorbrewer2.org/).
...
For this chapter's exercises, use the `covidcases` data set that we first introduced in [Chapter -@sec-transformations-summaries] to recreate some plots. These are complex plots, so try to build them up one step at a time and just try to get as close as possible to the given examples.

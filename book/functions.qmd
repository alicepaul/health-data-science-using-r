...
We can add to this function by instead adding our first argument called `name` which is a string and then printing "Hello, \[name\]!". Below, we use the `paste0()` function which concatenates the string arguments into a single string.
...
If we call this function with no arguments, it will use all the default values.

```{r}
dist_to_origin()
```

However, if we call the function with one argument, it will assume this first argument is `x`. Similarly, it will assume the second value is `y` and the third value is `z`. If we want to give the arguments without worrying about the order, we can specify them using their names (see the last line below).
...
If we want to use the created function, we can return it. In the updated example below we return an anonymous function. By doing so, we create a unique function for each `x` value.
...
Each comment should start with a pound sign and back-tick. The first block of lines is the introduction and the first line of the comment block is reserved for the title. This is the first information we want the user to know about the function and should be a single line. For all other information besides the title, we will use certain tags.
...
In this case, our input to this function will be a data frame which could contain a mixture of numeric and categorical columns and a threshold correlation value with a default value of 0.6. We want to return a data frame with the pairs and their correlation. This gives us a template for our documentation.
...
Let's start with step 1. Before putting code into our function, we are going to test our steps on our example data. To do so, we will use the `select_if()` function from the **dplyr** package.

```{r}
df_numeric <- select_if(test_df, is.numeric)
head(df_numeric)
```
Great, this worked! Next we need to find the strong correlations. Now, we can use the correlation function to find the correlations. We then want to iterate through all pairs to check if the absolute value of the correlation is above our threshold. We will use a loop for this. In our first attempt, we create a nested for loop where `i` will be one column and `j` will be our second column in the pair. We can see we must have some mistakes because we are getting pairs where `i` and `j` are equal to each other and we are also getting zeros. Note how we are using print statements. This helps us to identify that we need to add parentheses for the first for loop and we need to update `j` to start at `i+1`. 
...
Let's try again. This time we will create a variable `n` which will be the number of columns. 
...
```
Next, we need to add an if statement to check whether there is a correlation above the given threshold. In this case to check if our code is working correctly, we will use a print statement for only those that meet the condition. 
...
We have sketched out our code more thoroughly so we have a good idea of how we want to compute our result. We will move to writing our function. Importantly, we need to make sure that we use our input arguments now rather than our test values. In the version below, we also add in a results data frame that we use to keep track of our results and we add an additional argument for how to deal with NA values in calculating the correlations. The output matches what we expect for our test data frame.
...
We can prevent unexpected behavior of our functions by using `stop()` functions to limit a function to be run on certain types of arguments. This is helpful if other people will be using your function or if you might forget any assumptions you built into the function. The `stop()` function stops the execution of the current expression and returns a message. In the example below, we check to make sure that the point given is a numeric vector. Further, we check to see whether the vector has length zero and return 0 if it does.
...
Below, we demonstrate some tests for the `dist_to_origin()` function. Our tests here focus on the format of the vector. We should test each function separately so we would write a separate batch of tests for the Euclidean distance function.
...
Below are a series of tests for the function `high_cor()` with different data frame sizes and types of columns. Unfortunately, not all the tests are working. Use your debugging skills to fix the function to pass the tests and write at least one additional test.
...
In the video below, we demonstrate writing a function to calculate this confidence interval using a top-down approach. We also show more debugging examples.

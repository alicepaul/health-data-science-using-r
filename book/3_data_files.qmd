...
Summarize the `PROMIS_SLEEP_DISTURB_V1_0` column both numerically and visually. Your results should look like the results in @fig-3-pq1.

![Summarizing a Column.](images/3-practicequestion1answer.png){#fig-3-pq1 fig-align="center" width="100%"}
...
Find the sum of each of the PROMIS measures across all patients using `apply()` and then using `colSums()`. Verify that these two methods return the same result, which is given in @fig-3-pq2.

![Summing Across Columns.](images/3-practicequestion2answer.png){#fig-3-pq2 fig-align="center" width="100%"}
...
pain$PAT_RACE[pain$PAT_RACE %in% c("ALASKA NATIVE", "AMERICAN INDIAN")] <- 
  "AI/AN"
table(pain$PAT_RACE)
```

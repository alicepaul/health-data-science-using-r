...
The `glm(formula, data, family)` function\index{R functions!glm()@\texttt{glm()}} in R is used to fit generalized linear models. The three main arguments we must specify to the function are the
...
If we want to understand more about the curve, we can use the `coords()`\index{R functions!coords()@\texttt{coords()}} function to find the coordinates for each threshold used to create the curve. The argument `x= "all"` specifies we want to find all thresholds, but we could also specify only to return local maxima.

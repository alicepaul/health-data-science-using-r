...
In this chapter, we will put some of our R skills together in a case study. This case study will focus on data cleaning and pre-processing. We will use the `tb_diagnosis_raw` data from the **HDSinRdata** package. This data contains information on 1,634 patients in rural South Africa who presented at a health clinic with tuberculosis-related symptoms and who were tested for tuberculosis (TB) using Xpert MTB/RIF. Our goal will be to clean this data to reflect the pre-processing described in @baik2020clinical. This paper uses this data to derive a simple risk score model for screening patients for treatment while awaiting Xpert results. We will use the **tidyverse** packages as well as the summary tables from **gtsummary**.
...
To start, read in the data and read the description of the original columns. Some things to note in the data documentation are the ways unknown, missing, or refused values are coded as well as how some of the columns are related to each other.
...
To start, we will select variables needed for our analysis. In particular, we drop columns related to the participation in the survey and about seeking care. Since some of these variables contain long or vague names, we also rename most of the variables.
...
Next, we similarly code our variables about HIV status, smoking, and whether the patient has ever been diagnoses with tuberculosis before. For these variables, if the patient answered that they did not know if their HIV status or had tested positive for TB, we code these as 0 to be consistent with the paper. 
...
The next variable we will clean is `education`. First, we need to code NA values correctly. We can then observe the distribution of years of education.
...
There are several variables in the data related to how long a person has experienced symptoms. Below, we can see that unit of the symptoms, recorded in `length_symp_unit_fac`, determines which other column is entered. For example, if `length_symp_unit_fac == 1`, then the only column without an NA value will be `length_symp_days_fc`. 

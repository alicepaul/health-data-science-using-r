...
There are several variables in the data related to how long a person has experienced symptoms. Below, we can see that the unit of the symptoms, recorded in `length_symp_unit_fac`, determines which other column is entered. For example, if `length_symp_unit_fac == 1`, then the only column without an NA value will be `length_symp_days_fc`. 

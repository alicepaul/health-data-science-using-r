...
We write all of the text and code that we would like to include in your report in this .qmd file, and then produce a nicely formatted report from this file by 'rendering' the file\index{Quarto!rendering}. You can either render to HTML, PDF, or WORD by clicking on the Render button ![](images/quarto_reports/render-icon.png){width="35pt"} from the toolbar at the top of the page. To update our format from PDF, we change the text in the top block to `format: html` or `format: word`.

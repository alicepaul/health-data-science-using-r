...
Find the sum of each of the PROMIS measures across all patients using `apply()` and using `colSums()`. Verify that these two methods return the same result.
...
1. Print summary statistics for the `PROMIS_PHYSICAL_FUNTION` and `PROMIS_ANXIETY` variables in this dataset. Describe the distributions of these variables in the context of this dataset, especially given that the scale for the PROMIS physical function and anxiety measures goes from 0 to 100. 
...
4. Create a vector of the total number of patients who reported pain in each of bodily pain regions. Then, create a data frame containing the minimum, median, mean, maximum, standard deviation, and variance of this vector.

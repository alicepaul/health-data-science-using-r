...
### Practice Question
Set the random seed to be `123`, and then generate 5 random numbers following a [uniform distribution](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/Uniform) with min 1 and max 5. Then, find the 0.15 quantile for this same distribution (it should be equal to 1.6).
...
```{r}
pnorm(5*12, mean = 64, sd = 2.2)
```

```{r}
1-pnorm(6*12, mean = 64, sd = 2.2)
```

```{r}
set.seed(10)

x_norm <- rnorm(500, mean=64, sd = 2.2)
quantile(x_norm, 0.15)
q_15 <- qnorm(0.15, mean=64, sd = 2.2)
q_15
```
...
3. Suppose that we have a model for how a certain drug affects patients. We believe that those who are not given the drug have a reduction in tumor size that is randomly distributed as $X\sim100*exp(mean = R)$, where $R$ follows a uniform distribution with mean 0 and sd 1. Further, we believe that those who are given the drug see a reduction in tumor size distributed as $X \sim 100*exp(mean = 2)$. Simulate 100 patients and plot distributions of reduction in tumor size for both groups. 

...
In binary logistic regression, we assume a binomial outcome and use the logit link function. We can specify this by setting `family = binomial`. By default, this will assume the link function is the logit function. Note that we can even use the `glm()` function to implement linear regression by setting `family = gaussian`. Using our example from [Chapter 10](https://alicepaul.github.io/r-for-health-data-science/book/10_linear_regression.html), running `glm(SBP~LEAD, data = nhanes_df, family= gaussian)` would be equivalent to `lm(SBP~LEAD, data = nhanes_df)`.
...
Create a calibration plot with 15 cuts for your model from the previous practice question. It should look like this:
...
The `glm()` function can be used to fit models for other possible families and non-binary outcomes. For example, we can fit models where the outcome might follow a Poisson distribution or negative binomial distribution by updating the `family` argument. Below, we fit a ???
...
1. Create a new variable `tobacco_use` representing any tobacco use in the past 30 days (including e-cigs, cigarettes, and/or cigars) as well as a new variable `perceived_tobacco_use` equal to the maximum of the perceived cigarette and e-cig use. Then, create a new data frame `nyts_sub` that contains these two new columns as well as columns for sex, grades in the past year, psych distress, and family affluence. <br> <br> Fit a logistic regression model with this new tobacco use variable as the outcome and all other selected variables as independent variables.

```{r}
nyts <- nyts %>% mutate(tobacco_use = ifelse(num_e_cigs + num_cigarettes + num_cigars > 0, 1, 0),
                       perceived_tobacco_use = pmax(perceived_cigarette_use, perceived_e_cig_use))
nyts_sub <- nyts %>% 
  dplyr::select(sex, grades_in_past_year, 
                psych_distress, family_affluence, 
                tobacco_use, perceived_tobacco_use) %>%
  na.omit()

mod <- glm(tobacco_use ~ sex + grades_in_past_year + psych_distress + 
           family_affluence + perceived_tobacco_use,
           data = nyts_sub, family = binomial)

summary(mod)
```

2. Perform stepwise selection on your model from Question 1 with `direction = "both"`, setting the upper scope of the model selection procedure to be a model including all two-way interactions and the lower scope to be a model including only an intercept. To specify all possible interactions you can use the formula `"tobacco_use ~ .^2"`. <br> <br> Use the `tidy()` function to display the exponentiated estimated coefficients for the resulting model along with a confidence interval.   

```{r}
mod_step <- step(mod, direction = "both", scope = list(lower = "tobacco_use ~ 1", upper = "tobacco_use ~ .^2"))
tidy(mod_step, exponentiate=TRUE, conf.int=TRUE)
```
...
new_data <- data.frame(sex = "Female", 
                       grades_in_past_year = "Mostly C's", 
                       psych_distress = "moderate", 
                       perceived_tobacco_use = 0.5)

predict(mod_step, newdata = new_data, type = "response")
```

...
1. Take a look at the code below. Why does this code fail? In other words, what causes the warning message?


Solve this problem in the following three ways and look at the results of each: 
  1. Add an id column that has a unique value for each row;
  2. Keep only distinct rows of the name and key values, and drop duplicate rows using the `distinct()` function; 
  3. Change the names of the duplicate rows to make all names unique by adding in last names. 

```{r}
#1 
people <- data.frame(
  name = c("Phillip", "Phillip", "Phillip", "Jessica", "Jessica"),
  feature = c("height", "age", "age", "age", "height"),
...
people <- people %>%
  distinct(name, feature, .keep_all = TRUE) %>%
  pivot_wider(names_from="name", values_from = "value")

#3
people <- data.frame(
  name = c("Phillip", "John", "Mark", "Jessica", "Margaret"),
  feature = c("height", "age", "age", "age", "height"),
  value = c(45, 186, 50, 37, 156)
)
pivot_wider(people, names_from="name", values_from = "value")
...
mob_reshape <- mobility %>% 
  ...
  filter(state %in% c("California", "Michigan", "Connecticut", "Rhode Island",
                      "Ohio", "New York", "Massachusetts")) %>%
  group_by(state, month) %>%
  summarize(avg_m50 = mean(m50, na.rm = T))
...
```{r}
sub_cases_wide <- pivot_wider(sub_cases, names_from=state, values_from=total_cases)
sub_cases_wide
...
left_join(sub_cases_wide, mob_reshape, by=c("month"))
```

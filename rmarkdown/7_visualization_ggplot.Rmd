...
Now we can start adding layers to our `ggplot` object. One type of layer is a **geom**. These layers create a geometric object. Below, we use the `geom_point()` to add a scatter plot layer. For this function, we need to specify the data and tell R how to use the data to create the scatter plot through the `aes()` function which creates an **aesthetic**. For a scatter plot, we need to at least specify the x-axis and y-axis in the aesthetic. Both the data and the aesthetic can either be specified in our initial `ggplot()` function, which will pass this information to all future layers, or in the `geom_point()` function itself. We specify the aesthetic in the geom function but also include two alternative ways to code the same image that are commented out. The image shows a fairly linear relationship between anxiety and depression.
...
We can now use the `geom_col()` function to create a bar plot of the proportions. By default, this function will stack the bars on top of each other when there is grouping. Try adding `position="dodge"` to the `geom_col()` function to place them side by side instead. 

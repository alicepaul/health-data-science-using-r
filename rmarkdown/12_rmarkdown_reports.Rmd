...
Now that you have these packages downloaded, opening a new R Markdown file is very similar to opening a new R file, which was covered in [Chapter 1](https://alicepaul.github.io/r-for-health-data-science/book/1_intro_to_r.html). Just like opening a new R file, you'll want to go to File -> New File, but instead of selecting 'R Script', you'll now select 'R Markdown...'. This should bring up a window that looks like the following:
...
The exercise for this chapter is to recreate the following pdf created from an R Markdown file. You will need to use the `breastcancer` dataset from the `RforHDSdata` package.
